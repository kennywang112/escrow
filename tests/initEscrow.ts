...
import { ESCROW_ACCOUNT_DATA_LAYOUT, EscrowLayout } from "./layout.ts";
...
export const initEscrow = async (
    ...
    //@ts-expect-error
    const XTokenMintAccountPubkey = new PublicKey((await connection.getParsedAccountInfo(initializerXTokenAccountPubkey, 'confirmed')).value!.data.parsed.info.mint);

    const privateKeyDecoded = privateKeyByteArray.split(',').map(s => parseInt(s));
    const initializerAccount = new Account(privateKeyDecoded);

    // create X token account that will be transfered to the PDA 
    ...
    const initTempAccountIx = createInitializeAccountInstruction(TOKEN_PROGRAM_ID, XTokenMintAccountPubkey, tempTokenAccount.publicKey, initializerAccount.publicKey);
    const transferXTokensToTempAccIx = createTransferInstruction(TOKEN_PROGRAM_ID, initializerXTokenAccountPubkey, tempTokenAccount.publicKey, initializerAccount.publicKey, [], amountXTokensToSendToEscrow);
    ...
    const createEscrowAccountIx = SystemProgram.createAccount({
        ...
        lamports: await connection.getMinimumBalanceForRentExemption(ESCROW_ACCOUNT_DATA_LAYOUT.span, 'confirmed'),
        fromPubkey: initializerAccount.publicKey,
